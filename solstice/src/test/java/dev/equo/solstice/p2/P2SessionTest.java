...
public class P2SessionTest {
	...
	public void testResolution() throws Exception {
		...
		resolution.exclude("a.jre.javase");
		resolution.exclude("org.eclipse.platform_root");
		resolution.exclude("org.eclipse.rcp_root");
		resolution.excludePrefix("tooling");
		resolution.resolve(session.getUnitById("org.eclipse.platform.ide.categoryIU"));
		var jars = resolution.jarsOnMavenCentral().stream().collect(Collectors.joining(","));
		...
		var notOnMavenCentral =
				resolution.jarsNotOnMavenCentral().stream()
						.map(unit -> unit.id)

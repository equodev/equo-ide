...
class BrandingImpl implements IdeHookInstantiated {
	private final IdeHookBranding data;
	Logger logger = LoggerFactory.getLogger(IdeHookBranding.class);

	public BrandingImpl(final IdeHookBranding data) {
		this.data = data;
	}
	...
	public void isClean(boolean isClean) throws Exception {
		if (OS.getRunning().isMac()) {
			System.setProperty("com.apple.mrj.application.apple.menu.about.name", data.title);
		}
	}
	...
	private Image loadImage(Display display, File file, String defaultResource) {
		try {
			...
			LoggerFactory.getLogger(IdeHookBranding.class)
					.warn("Unable to load image " + file.getAbsolutePath() + ", falling back to default.", e);
		}
		try (var input =
	...
	public void afterDisplay(Display display) {
		...
		Image icon = loadImage(Display.getDefault(), data.icon, "dev/equo/ide/equo_icon.png");

	...
	void configure() {
		...
		for (Map.Entry<String, P2Model> entry : configurations.entrySet()) {
			...
			for (String mavenCoord : query.getJarsOnMavenCentral()) {
				project.getLogger().debug("Adding Maven resolved dependency: "+mavenCoord); //TODO: make debug
			...
			for (File coord : query.getJarsNotOnMavenCentral()) {
				project.getLogger().debug("Adding non-maven dependency: "+coord.getName()); //TODO: make debug
			}
